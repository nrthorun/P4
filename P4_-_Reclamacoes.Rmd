...
library(knitr)
library(kableExtra)
#alinha as figuras à direita
knitr::opts_chunk$set(fig.align = "center")
#seta as tabelas como html, para não ter que setar em todas tabelas
...
#arruma os caracteres especiais dos estados
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#esta estrutura utilizando os pipes da biblioteca 'dplyr' será usada
#em todo o projeto
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa de cada faixa etária
var <- rec %>%
  select(`Faixa Etária`) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa de cada segmento
var <- rec %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa de cada grupo problema
var <- rec %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa das reclamações
#resolvidas ou não
var <- rec %>%
  select(`Resolvida`) %>%
  group_by(`Resolvida`) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa das reclamações
#resolvidas, não resolvidas ou não avaliadas
var <- rec %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa para cada dia do
#tempo de resposta
var <- rec %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#faz a média do tempo de resposta omitindo os NA's
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa para cada nota
var <- na.omit(rec) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#faz a nota média omitindo os NA's
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa para estado
var <- rec %>%
  select(`UF`) %>%
  group_by(`UF`) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa para cada empresa
var <- rec %>%
  select(`Nome`) %>%
  group_by(`Nome`) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa por segmento e por sexo
var <- rec %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa por segmento e por
#faixa etária
#cria a coluna 'pos' que dá a posição do rótulo de dados no gráfico
var <- rec %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa por faixa etária e sexo
#cria a coluna 'pos' que dá a posição do rótulo de dados no gráfico
var <- na.omit(rec) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#faz o teste de correlação entre tempo de resposta e nota
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#faz o boxplot de tempo de resposta por nota
var <- na.omit(rec)
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca a média do tempo de resposta por nota
var <- na.omit(rec) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#faz a correlação entre 'resolvida' e nota
var <- na.omit(rec)
...
#busca no subset recém criado o número de ocorrências e a frequência
#relativa dos grupos de problema
var <- segsub %>%
...
ggplot(var, aes(x = reorder(`Grupo Problema`, -n), y = n)) +
  ...
  geom_text(aes(label = paste0(sprintf("%.0f", relFreq * 100),"%"),
                vjust = -0.5), color = "Black") +
  #replica o gráfico para cada segmento de mercado do subset
...
#busca no subset o número de ocorrências por grupo problema e problema
var <- segsub %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa por faixa etária, sexo
#e segmento
#cria a coluna 'pos' que dá a posição do rótulo de dados no gráfico
var <- rec %>%
  select(`Segmento de Mercado`, Sexo, `Faixa Etária`) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#faz a nota média por reclamação resolvida e não resolvida
var <- na.omit(rec) %>%
  select(`Nota do Consumidor`, Resolvida) %>%
  group_by(Resolvida) %>%
  summarize(media = mean(`Nota do Consumidor`)) 

#mostra a variável acima como tabela
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#faz o tempo médio de resposta por reclamação resolvida e não resolvida
var <- na.omit(rec) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca a nota média por por segmento para as reclamações resolvidas e não
#resolvidas
var <- na.omit(segsub) %>%
...
ggplot(segsub, aes(x = `Grupo Problema`, y = `Nota do Consumidor`)) +
  ...
  #seta as cores dos segmentos manualmente
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca as notas médias por segmento e por grupo problema
#cria uma coluna para cada grupo problema
var <- na.omit(segsub) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#cria um subset apenas com as reclamações de telecomunicações
telecom <- subset(rec, `Segmento de Mercado` ==
  'Operadoras de Telecomunicações (Telefonia, Internet, TV por assinatura)')

#busca o índice de solução
var <- telecom %>%
...
#busca as reclamações respondidas
var <- telecom %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#busca o número de ocorrências e a frequência relativa por empresa e por
#grupo problema
#cria a coluna 'pos' que dá a posição do rótulo de dados no gráfico
var <- telecom %>%
...
#busca o número de ocorrências por empresa e por estado
var <- telecom %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#cria subset apenas com as reclamações de comércio eletrônico
ecomm <- subset(rec, `Segmento de Mercado`=='Comércio Eletrônico')

#cria índice de solução
var <- ecomm %>%
  select(`Avaliação Reclamação`, `Nome`, `Nota do Consumidor`) %>%
  group_by(`Nome`) %>%
  summarize(n = length(`Avaliação Reclamação`),
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#cria um subset apenas com as reclamações de bancos
banks <- subset(rec, `Segmento de Mercado` ==
                  'Bancos, Financeiras e Administradoras de Cartão')

#cria o índice de solução
var <- banks %>%
  select(`Avaliação Reclamação`, `Nome`, `Nota do Consumidor`) %>%
  group_by(`Nome`) %>%
  summarize(n = length(`Avaliação Reclamação`),
...
#cria o tempo médio de resposta  
  var <- banks %>%
  select(`Tempo Resposta`, `Nome`, `Nota do Consumidor`) %>%
  filter(complete.cases(`Tempo Resposta`)) %>%
  group_by(`Nome`) %>%
...
#cria as reclamações respondidas
  var <- banks %>%
  select(`Respondida`, `Nome`, `Nota do Consumidor`) %>%
  group_by(`Nome`) %>%
  summarize(n = length(Respondida), m = sum(Respondida == 'S')) %>%
